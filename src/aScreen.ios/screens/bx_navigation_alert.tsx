...
export const AllCommentScreen = ({ navigation, route }: RootStackScreenProps<"全部评论">) => {
   const { vod_id, vod_name, commentItems, vod_douban_id } = route.params;
   ...
   const fetchComments = () =>
      RCanvas.getReviewDetails(vod_douban_id.toString() ?? "").then(
         (data) => {
   ...
   const { data: onlineComments, isFetching: isFetchingComments } = useQuery({
      queryKey: ["relatedComments", vod_id],
      queryFn: () => fetchComments(),
   });

   const locCommentId = "userComment" + vod_id;
   const allComment = commentItems ?? onlineComments ?? [];
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
         ...
         >
            <TitleWithBackButtonHeader
               title={vod_name + " - 全部评论 (" + allComment.length + ")"}
               backBtnStyle={{
                  left: 30,
               }}
            />
