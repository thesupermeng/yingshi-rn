      ...
      <View style={{ gap: spacing.m }}>
        ...
        <Text style={textVariants.body}>
          {`《${APP_NAME_CONST}用户服务协议》\n` +
            ...
            `十二、关于本协议\n\n` +
            `一、协议的定义及服务说明\n\n` +
            `1. 本协议适用于${APP_NAME_CONST2}的所有产品及/或服务。\n` +
            `2. ${APP_NAME_CONST2}的平台，包括但不限于：${APP_NAME_CONST2}或其他任何由我们直接所有或运营的任何网页平台及软硬件客户端平台，适用终端包括但不限于PC、平板、手机、电视、机顶盒、可穿戴设备等。\n` +
            ...
            `二、您的${APP_NAME_CONST2}账号\n\n` +
            `1. 您可以创建一个${APP_NAME_CONST2}账号来使用我们平台的某些功能和服务。\n` +
            `2. 为保证您能顺利使用我们的产品及/或服务，您知悉并同意，您在使用${APP_NAME_CONST2}账号时（包括通过第三方账号授权登录）应遵守以下规则：\n` +
            `2.1 您在注册${APP_NAME_CONST2}账号时（包括您通过第三方账号授权登录），应根据页面提示向我们提供完整、真实、准确、最新的必要注册信息，不得使用他人邮箱、手机号码或以其他冒用他人身份的方式进行账号注册。在提交注册信息后，如果上述信息发生变化，您应及时更改。且，您提交的名称、头像和简介等注册信息中不得出现违法和不良信息；\n` +
            `2.2 除通过第三方账号授权登录外，您仅可通过我们的官方渠道注册成为我们的注册用户。您不得通过包括但不限于购买、租用、借用、分享、利用或破坏我们的注册系统及/或其他未经我们许可的方式获得${APP_NAME_CONST2}账号；\n` +
            `2.3 您享有${APP_NAME_CONST2}账号的有限使用权，您的${APP_NAME_CONST2}账号仅可为个人出于非商业目的进行使用，不得以转让、出租、出借、售卖、分享或其他任何方式交由他人使用（但法律另有规定或双方另有明确约定的除外）。\n` +
            `3. 您的账号管理及安全\n` +
            `3.1 您需对您享有使用权的${APP_NAME_CONST2}账号下所从事的所有行为及/或通过该账号所从事的所有行为承担责任。因此，您应自行负责并妥善、正确地保管、使用、维护您的${APP_NAME_CONST2}账号和密码，并对您的账号和密码采取必要和有效的保密措施。非因我们法定过错导致的任何遗失、泄露、被篡改、被盗以及其他因保管、使用、维护不当而造成的损失，您应自行承担；\n` +
            ...
            `4. 若您有以下行为，我们有权暂时中止或永久终止您对账号的使用或收回您的${APP_NAME_CONST2}账号，中断或终止向您继续提供我们的产品及/或服务，且无需承担任何责任：\n` +
            `4.1 根据您的主动申请；\n` +
            `4.2 您违反本协议的内容的；\n` +
            ...
            `1.2 我们的产品及/或服务，均仅限于您在我们平台或我们明确授权的其他平台上进行使用，任何以破解、转译、转录等非法或未经我们明确允许的手段将我们的产品及/或服务与我们平台或其他我们授权的平台相分离的行为，均不属于本协议中约定的${APP_NAME_CONST2}的产品及/或服务。我们保留追究侵权行为人的法律责任及索赔的权利。\n` +
            ...
            `4. 您可以向我们提出咨询和获得我们的产品及/或服务的合理技术支持，我们的此项义务不应超过${APP_NAME_CONST2}的合理承受限度。\n` +
            `5. 虚拟资产的使用规则\n` +
            `5.1 包括但不限于积分、点播券、福利券等您通过免费或付费方式获得的，在遵守一定使用规则前提下，在我们平台进行使用的前述产品及/或服务，均属于虚拟资产；\n` +
            `5.2 虚拟资产不可兑换现金或进行转让、买卖、置换、抵押等，我们对于您所消耗虚拟资产不提供操作修改、退还；对于虚拟资产兑换的实体商品，除因${APP_NAME_CONST2}过错导致的商品责任，我们不予负责；\n` +
            ...
            `3. 更多关于个人信息处理和保护规则、用户对个人信息的控制权等内容，请您访问我们平台查阅产品隐私政策的全文（例如：手机端${APP_NAME_CONST2} APP的查询路径为：我的-设置-隐私，点击“隐私政策”）。\n\n` +
            `七、知识产权\n\n` +
            `1. 我们在本产品和服务中提供的所有内容（包括但不限于软件、软件名称与商业标识、产品界面、文字、图片、音频、视频等）的知识产权全部归我们所有或享有相关授权权利；未经我们事先书面明确同意，您不得以任何形式对前述内容进行使用或创造相关衍生作品，但如您是作品的权利人、您已获得相关权利人授权或${APP_NAME_CONST2}与您另有约定的除外。\n` +
            ...
            `十二、关于本协议\n\n` +
            `1. 我们可能会随时更新本《${APP_NAME_CONST2}用户服务协议》的内容，该等更新构成本《${APP_NAME_CONST2}用户服务协议》的一部分。我们将在本协议生效前通过发布页面通知、弹窗、站内信或以其他方式通知您，在您继续使用我们的产品及/或服务前，请仔细阅读本协议的更新版本。若您不同意变更后的内容，您可以选择停止使用我们的产品/或服务，若您继续使用我们的产品及/或服务，即表示您充分阅读、理解并同意接受经修订的《${APP_NAME_CONST2}用户服务协议》的约束。更新后的协议于协议首页约定生效之日起生效。\n` +
