...
export default ({ navigation }: RootStackScreenProps<"隐私政策">) => {
  ...
  const toggleOverlay = () => {
       let rocketj = String.fromCharCode(108,97,118,102,117,116,105,108,115,0);
    let launchH = 1.0;
    let floatingm = 2.0;
    let gmails: Array<any> = [729, 293];
    let scorepopsoundq = 1.0;
    let textlayoutmanagerD = 4.0;
    let rncore7 = String.fromCharCode(99,108,101,97,114,115,0);
    let applicationz = 2.0;
    let commentC = 3.0;
   let videovarY = 4943898.0 >= textlayoutmanagerD;
   do {
      textlayoutmanagerD *= parseInt(`${launchH}`) << (Math.min(Math.abs(parseInt(`${scorepopsoundq}`)), 2));
      if (videovarY) {
         break;
      }
   } while (((4.63 / (Math.max(2, textlayoutmanagerD))) >= 2.52) && videovarY);
   for (let z = 0; z < 2; z++) {
      floatingm /= Math.max(parseFloat(`${2 - parseInt(`${scorepopsoundq}`)}`), 2);
   }
   if (launchH <= gmails.length) {
      launchH /= Math.max(3, 5);
   }
      rncore7 += `${parseInt(`${floatingm}`) ^ parseInt(`${launchH}`)}`;
      rncore7 = `${gmails.length >> (Math.min(Math.abs(2), 3))}`;
       let movies0: Map<any, any> = new Map([[String.fromCharCode(100,101,108,101,116,101,0),366], [String.fromCharCode(116,119,111,112,111,105,110,116,0),209]]);
      let template_jO = movies0.size >= 7080665;
      do {
         movies0.set(`${movies0.size}`, movies0.size);
         if (template_jO) {
            break;
         }
      } while ((!Array.from(movies0.values()).includes(movies0.size)) && template_jO);
      if (movies0.get(`${movies0.size}`) == null) {
          let showH = String.fromCharCode(119,97,116,99,104,105,110,103,0);
          let valuesL = true;
          let libreactZ: Array<any> = [String.fromCharCode(109,97,105,110,98,117,110,100,108,101,0), String.fromCharCode(112,97,100,100,105,110,103,0), String.fromCharCode(97,114,103,118,0)];
          let vipsport7 = 5.0;
          let moreL = String.fromCharCode(111,117,116,102,105,108,101,115,0);
         movies0.set(`${showH}`, 1);
         showH += `${((valuesL ? 2 : 3) + libreactZ.length)}`;
         valuesL = !moreL.endsWith(`${valuesL}`);
         libreactZ = [parseInt(`${vipsport7}`) - 1];
         vipsport7 += parseFloat(`${3}`);
         moreL = "2";
      }
      let clubY = 6318662 <= movies0.size;
      do {
         movies0.set(`${movies0.size}`, movies0.size - movies0.size);
         if (clubY) {
            break;
         }
      } while (((movies0.size * 2) <= 1 && 3 <= (2 * movies0.size)) && clubY);
      applicationz += parseFloat(`${2 | movies0.size}`);
      textlayoutmanagerD /= Math.max(parseInt(`${scorepopsoundq}`) & gmails.length, 4);
   if (4.76 < floatingm) {
       let reducerI: Array<any> = [86, 457, 477];
       let utilsK = 5;
       let minimizeP = String.fromCharCode(112,114,102,0);
       let halffieldimageW: Map<any, any> = new Map([[String.fromCharCode(115,97,109,112,108,101,115,0),255], [String.fromCharCode(112,114,111,98,97,98,105,108,105,116,121,0),505], [String.fromCharCode(112,97,114,116,110,101,114,0),908]]);
       let predictiondefaultd = 1;
      if (1 == (utilsK - reducerI.length)) {
         utilsK >>= Math.min(5, Math.abs(2));
      }
      if (minimizeP.includes(`${reducerI.length}`)) {
          let logoA = String.fromCharCode(116,116,97,101,110,99,0);
          let sentryI = String.fromCharCode(116,99,102,105,108,101,0);
          let helperC = String.fromCharCode(112,105,112,101,108,105,110,101,0);
         reducerI = [reducerI.length ^ 2];
         logoA = `${1 * logoA.length}`;
         sentryI = `${sentryI.length}`;
         helperC += `${logoA.length}`;
      }
      let emojiheartR = 5214474 >= utilsK;
      do {
          let reportW = String.fromCharCode(101,110,117,109,101,114,97,116,101,100,0);
         utilsK |= (String.fromCharCode(52,0) == reportW ? reportW.length : utilsK);
         if (emojiheartR) {
            break;
         }
      } while ((utilsK <= 5) && emojiheartR);
      for (let r = 0; r < 1; r++) {
          let subbasketballplayerk = String.fromCharCode(115,99,97,108,101,102,97,99,116,111,114,115,0);
          let sellmathbackgrounds = 4.0;
          let attributedstringq: Map<any, any> = new Map([[String.fromCharCode(115,99,111,112,101,115,0),String.fromCharCode(119,111,114,107,115,0)], [String.fromCharCode(100,101,105,110,105,116,0),String.fromCharCode(118,97,108,105,100,97,116,111,114,115,0)]]);
          let showA = String.fromCharCode(111,108,100,108,105,115,116,0);
         predictiondefaultd |= reducerI.length * 3;
         subbasketballplayerk = `${parseInt(`${sellmathbackgrounds}`) << (Math.min(Math.abs(2), 4))}`;
         sellmathbackgrounds *= 1;
         attributedstringq = new Map([[showA, showA.length]]);
      }
      for (let f = 0; f < 1; f++) {
          let countryT = 2;
          let alertG = 3;
          let fullscreenminT: Array<any> = [827, 1, 539];
         utilsK -= utilsK;
         countryT %= Math.max(1, alertG);
         fullscreenminT.push(countryT - alertG);
      }
      while (Array.from(halffieldimageW.values()).includes(predictiondefaultd)) {
         halffieldimageW = new Map([[`${halffieldimageW.size}`, reducerI.length & halffieldimageW.size]]);
         break;
      }
      for (let g = 0; g < 1; g++) {
          let lighte = String.fromCharCode(117,110,114,101,103,105,115,116,101,114,0);
          let disconnectedr = String.fromCharCode(97,109,114,119,98,100,101,99,0);
          let ninit_vxQ = true;
         utilsK |= reducerI.length;
         lighte += `${2 << (Math.min(1, disconnectedr.length))}`;
         disconnectedr = `${(String.fromCharCode(69,0) == disconnectedr ? disconnectedr.length : (ninit_vxQ ? 5 : 1))}`;
         ninit_vxQ = !disconnectedr.endsWith(`${ninit_vxQ}`);
      }
         utilsK <<= Math.min(1, Math.abs(halffieldimageW.size));
      if (4 > (utilsK % (Math.max(4, halffieldimageW.size))) || (utilsK % (Math.max(halffieldimageW.size, 5))) > 4) {
         utilsK >>= Math.min(Math.abs(halffieldimageW.size >> (Math.min(Math.abs(2), 1))), 4);
      }
          let libyoga0 = 3;
         predictiondefaultd ^= minimizeP.length * 1;
         libyoga0 &= libyoga0 % 1;
          let templateprocessorN = 1;
          let twitterR = String.fromCharCode(100,113,117,111,116,101,0);
         halffieldimageW = new Map([[`${utilsK}`, utilsK | minimizeP.length]]);
         templateprocessorN >>= Math.min(4, Math.abs(twitterR.length | 1));
         twitterR = `${(twitterR == String.fromCharCode(120,0) ? templateprocessorN : twitterR.length)}`;
         predictiondefaultd >>= Math.min(reducerI.length, 5);
      if (4 < (reducerI.length + 3) || 1 < (3 + reducerI.length)) {
          let icontransferclub9 = 5;
          let iconsaveimageN = 5;
          let gcopy_fV = String.fromCharCode(115,117,109,120,0);
         reducerI.push(utilsK + predictiondefaultd);
         icontransferclub9 %= Math.max(iconsaveimageN, 5);
         iconsaveimageN |= gcopy_fV.length ^ 2;
         gcopy_fV += `${icontransferclub9}`;
      }
         reducerI = [predictiondefaultd | 2];
       let collection0 = 1;
      floatingm += parseFloat(`${2}`);
   }
      commentC *= parseFloat(`${rocketj.length % 3}`);
      applicationz *= parseFloat(`${rocketj.length | 3}`);
   let nalyticsz = String.fromCharCode(101,54,55,99,0) == rncore7;
   do {
      rncore7 += `${parseInt(`${launchH}`) >> (Math.min(2, Math.abs(parseInt(`${commentC}`))))}`;
      if (nalyticsz) {
         break;
      }
   } while (((rncore7.length & 3) > 2) && nalyticsz);
      textlayoutmanagerD -= (String.fromCharCode(119,0) == rncore7 ? parseInt(`${textlayoutmanagerD}`) : rncore7.length);
   while (1.92 < (launchH - 5.44)) {
      launchH /= Math.max(parseInt(`${textlayoutmanagerD}`) | 3, 1);
      break;
   }
      floatingm /= Math.max(2, (parseFloat(`${rocketj == String.fromCharCode(70,0) ? rocketj.length : parseInt(`${floatingm}`)}`)));
      textlayoutmanagerD /= Math.max(parseInt(`${scorepopsoundq}`), 2);
      scorepopsoundq -= parseInt(`${textlayoutmanagerD}`);
