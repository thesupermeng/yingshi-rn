import vars from './tt_mbjscommon_source';
...
export const parseStringToDate = dateStr => {
  ...
  if (typeof dateStr === 'string') {
    ...
    return new Date(parseInt(day[0]), parseInt(day[1]) - 1, parseInt(day[2]));
  }
...
export const liveRoomName = async matchId => {
  ...
  const res = await ttDescTerms.call(Url.liveRoomDetail, params, 'GET');
  if (res.success) {
    if (res.data.id === 0) {
...
export const createShareLink = async (path, id) => {
  ...
  const response = await ttDescTerms.call(Url.createShare, params);
  if (response.success) {
    const key = response.data;
    const shareLink = `${ttCalendarAdult.instance.config?.shortUrlDomain}/${key}`;
    return shareLink;
  } else {
    return '';
  }
