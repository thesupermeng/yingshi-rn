...
export default ({navigation, route}: RootStackScreenProps<'播放'>) => {
  ...
  const BTN_SELECT_WIDTH = useMemo(() => {
    ...
    const val =
      textVariants?.header?.fontSize === undefined
        ? 16
        : textVariants.header.fontSize;
  ...
  const NUM_PER_ROW = useMemo(
    () => Math.max(Math.floor(windowDim / (BTN_SELECT_WIDTH + 10)), 1),
    [windowDim, BTN_SELECT_WIDTH],
  );
  const BTN_MARGIN_RIGHT = useMemo(() => {
    let mr = 0;
    if (NUM_PER_ROW > 1) {
      mr = Math.floor(
        (windowDim - NUM_PER_ROW * BTN_SELECT_WIDTH) / (NUM_PER_ROW - 1),
      );
    }
  ...
  const NUM_OF_ROWS = useMemo(
    () =>
      vod?.vod_play_list
        ? Math.floor(vod.vod_play_list.url_count / NUM_PER_ROW)
        : 0,
    [vod, NUM_PER_ROW],
  );
  const ROW_HEIGHT = useMemo(() => {
    const height =
      textVariants?.header?.fontSize === undefined
        ? 22
        : textVariants.header.fontSize + 6;
