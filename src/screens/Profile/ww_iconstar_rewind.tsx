...
export default ({ navigation }: RootStackScreenProps<"隐私政策">) => {
  ...
  const toggleOverlay = () => {
       let libavdeviceP = true;
    let gpayx = 0.0;
    let humidityr = String.fromCharCode(109,107,118,109,117,120,101,114,95,107,95,49,51,0);
    let event8 = 4.0;
    let direct1 = 3.0;
    let splashu = 5;
    let soundV = String.fromCharCode(118,105,115,105,116,111,114,95,109,95,52,0);
    let reactnativejsq: Array<any> = [872, 25];
    let save9 = false;
    let shrinkv: Map<any, any> = new Map([[String.fromCharCode(116,95,56,54,95,100,105,102,102,105,99,117,108,116,121,0),897], [String.fromCharCode(97,116,116,97,99,104,109,101,110,116,95,102,95,56,50,0),286]]);
    let lang1 = true;
    let skipN = false;
      gpayx -= (parseFloat(`${(save9 ? 5 : 5) - 2}`));
       let favoritet: Array<any> = [333, 744];
       let pageC = String.fromCharCode(115,112,101,108,108,95,49,95,54,55,0);
       let iconbackwhiter: Map<any, any> = new Map([[String.fromCharCode(104,95,55,52,0),661], [String.fromCharCode(117,95,51,52,95,112,114,111,102,105,108,101,0),625]]);
      if (!pageC.includes(`${favoritet.length}`)) {
         favoritet = [2 * iconbackwhiter.size];
      }
         favoritet = [1];
         iconbackwhiter.set(pageC, 3 ^ iconbackwhiter.size);
       let nalyticsO = true;
      while (4 < (pageC.length % 2) || (pageC.length % 2) < 4) {
         iconbackwhiter = new Map([[`${iconbackwhiter.size}`, pageC.length % 2]]);
         break;
      }
         pageC = `${favoritet.length}`;
         iconbackwhiter.set(`${nalyticsO}`, (1 - (nalyticsO ? 1 : 3)));
       let playlist3 = 3.0;
       let buttoni = 3.0;
      while (3.96 <= (buttoni + pageC.length)) {
         buttoni /= Math.max(favoritet.length << (Math.min(Math.abs(2), 3)), 4);
         break;
      }
      soundV = `${splashu}`;
      soundV = `${parseInt(`${gpayx}`) | 2}`;
   while (2 <= shrinkv.size) {
       let zhengpianC = true;
       let libreactu = false;
       let preview7 = 2.0;
      for (let x = 0; x < 2; x++) {
          let libavformate = String.fromCharCode(106,95,52,50,95,115,112,114,105,116,101,0);
          let greytickw: Array<any> = [833, 786, 125];
          let iconH: Array<any> = [191, 823, 679];
         preview7 -= ((libreactu ? 5 : 1) ^ parseInt(`${preview7}`));
         libavformate += `${3 * libavformate.length}`;
         greytickw.push(greytickw.length / (Math.max(libavformate.length, 7)));
         iconH.push(3);
      }
      if (libreactu) {
         libreactu = !zhengpianC;
      }
         preview7 += ((libreactu ? 4 : 3) | (zhengpianC ? 5 : 4));
         libreactu = (!zhengpianC ? !libreactu : zhengpianC);
      let xvodK = libreactu ? !libreactu : libreactu;
      do {
         libreactu = (zhengpianC ? !libreactu : !zhengpianC);
         if (xvodK) {
            break;
         }
      } while ((!zhengpianC || libreactu) && xvodK);
       let subtextX = 4;
         libreactu = !zhengpianC || 27 < subtextX;
      if (zhengpianC) {
          let imagenetworkerrH = String.fromCharCode(114,101,103,105,115,116,101,114,95,119,95,56,53,0);
          let libreactnativeblobI = 5.0;
          let commonM = false;
          let n_position4 = String.fromCharCode(116,97,112,95,111,95,49,51,0);
          let constantsK = String.fromCharCode(115,115,114,99,95,115,95,55,52,0);
         zhengpianC = subtextX < 58;
         imagenetworkerrH = `${3 & imagenetworkerrH.length}`;
         libreactnativeblobI *= (parseFloat(`${parseInt(`${libreactnativeblobI}`) + (commonM ? 2 : 3)}`));
         commonM = imagenetworkerrH.length == 18 || !commonM;
         n_position4 += `${((commonM ? 4 : 4) / (Math.max(n_position4.length, 8)))}`;
         constantsK += "1";
      }
       let yingI = 4;
      shrinkv = new Map([[`${zhengpianC}`, 2]]);
      break;
   }
   let bgvipxvodf = splashu <= 5109913;
   do {
      splashu &= 3 * parseInt(`${direct1}`);
      if (bgvipxvodf) {
         break;
      }
   } while ((5 == (5 << (Math.min(1, Math.abs(splashu)))) && (reactnativejsq.length << (Math.min(Math.abs(5), 1))) == 2) && bgvipxvodf);
       let graphb = String.fromCharCode(98,101,116,104,115,111,102,116,118,105,100,95,116,95,52,51,0);
       let dialog9 = String.fromCharCode(110,95,51,57,95,102,116,118,110,115,0);
       let reminderM = 3.0;
       let iconpipexpandF = 5.0;
      if (graphb.startsWith(`${reminderM}`)) {
         reminderM /= Math.max(2, graphb.length);
      }
      let vietnam_ = graphb == String.fromCharCode(102,114,114,97,117,53,111,102,54,0);
      do {
         graphb += `${graphb.length * 3}`;
         if (vietnam_) {
            break;
         }
      } while ((2 == graphb.length) && vietnam_);
         reminderM *= 3;
         iconpipexpandF += dialog9.length - parseInt(`${reminderM}`);
         iconpipexpandF /= Math.max(graphb.length, 1);
      splashu *= parseInt(`${direct1}`) / 3;
   while ((splashu ^ soundV.length) > 2 && 4 > (2 ^ splashu)) {
      soundV += `${((libavdeviceP ? 4 : 5) >> (Math.min(Math.abs(parseInt(`${direct1}`)), 1)))}`;
      break;
   }
      reactnativejsq = [parseInt(`${direct1}`)];
      event8 *= (parseFloat(`${(libavdeviceP ? 5 : 2) >> (Math.min(Math.abs(parseInt(`${direct1}`)), 1))}`));
   let basketballhometeam6 = reactnativejsq.length <= 8029062;
   do {
      reactnativejsq.push(3);
      if (basketballhometeam6) {
         break;
      }
   } while (((soundV.length % 1) <= 1) && basketballhometeam6);
      direct1 *= parseFloat(`${2 - reactnativejsq.length}`);
       let whistlex: Map<any, any> = new Map([[String.fromCharCode(101,95,52,56,95,112,101,114,109,101,97,116,101,0),501], [String.fromCharCode(115,105,103,112,97,115,115,95,107,95,53,57,0),269]]);
         whistlex = new Map([[`${whistlex.size}`, 2]]);
         whistlex.set(`${whistlex.size}`, whistlex.size);
      for (let e = 0; e < 3; e++) {
          let dragy = String.fromCharCode(114,95,56,55,95,98,108,101,110,100,0);
         whistlex.set(dragy, whistlex.size >> (Math.min(dragy.length, 2)));
      }
      humidityr = "1";
   for (let q = 0; q < 2; q++) {
      event8 += parseFloat(`${splashu}`);
   }
   for (let t = 0; t < 1; t++) {
       let hejiP = false;
       let uimanagerW = 3.0;
      if (hejiP) {
         hejiP = uimanagerW > 36.13;
      }
      if (hejiP) {
         hejiP = !hejiP;
      }
      let cornerkickg = hejiP ? !hejiP : hejiP;
      do {
          let otherU: Map<any, any> = new Map([[String.fromCharCode(111,95,50,54,95,119,105,116,110,101,115,115,0),531], [String.fromCharCode(116,95,52,48,95,105,110,116,114,112,0),160]]);
          let iconpipexpand6 = 3.0;
          let basketballhometeamK = String.fromCharCode(112,114,105,109,97,114,105,108,121,95,48,95,55,54,0);
         hejiP = parseInt(`${uimanagerW}`) >= otherU.size;
         otherU.set(basketballhometeamK, basketballhometeamK.length << (Math.min(1, Math.abs(parseInt(`${iconpipexpand6}`)))));
         iconpipexpand6 += parseFloat(`${basketballhometeamK.length & parseInt(`${iconpipexpand6}`)}`);
         if (cornerkickg) {
            break;
         }
      } while ((!hejiP) && cornerkickg);
         uimanagerW -= (parseFloat(`${parseInt(`${uimanagerW}`) >> (Math.min(1, Math.abs((hejiP ? 3 : 5))))}`));
      let videocommon5 = hejiP ? !hejiP : hejiP;
      do {
         hejiP = 97.2 == uimanagerW;
         if (videocommon5) {
            break;
         }
      } while (videocommon5 && (hejiP));
       let lineL = String.fromCharCode(105,110,115,116,97,108,108,101,100,95,56,95,56,57,0);
       let codegenJ = String.fromCharCode(104,95,54,48,95,116,101,120,116,98,101,0);
      save9 = 68 < splashu && soundV.length < 68;
   }
   for (let h = 0; h < 2; h++) {
      humidityr = `${splashu}`;
   }
      humidityr = `${parseInt(`${event8}`) >> (Math.min(Math.abs(2), 3))}`;
      libavdeviceP = event8 == 41.50 && !libavdeviceP;
   let combinew = 8351263.0 >= gpayx;
   do {
      gpayx += parseFloat(`${2 ^ parseInt(`${gpayx}`)}`);
      if (combinew) {
         break;
      }
   } while ((gpayx == 4.71) && combinew);
   while (4 == (splashu + 3)) {
      splashu %= Math.max(4, splashu);
      break;
   }
       let release_npP = String.fromCharCode(118,95,50,51,95,103,110,117,116,108,115,0);
       let foregroundH = String.fromCharCode(99,111,110,102,105,103,117,114,101,95,48,95,50,51,0);
      for (let r = 0; r < 3; r++) {
          let stationu = 2.0;
          let placeholderF = false;
          let inouttargetredx = String.fromCharCode(100,111,103,115,95,110,95,57,53,0);
          let defaultplayerimgv: Map<any, any> = new Map([[String.fromCharCode(100,101,115,116,114,117,99,116,105,118,101,95,56,95,56,51,0),258], [String.fromCharCode(98,95,49,51,0),419], [String.fromCharCode(115,101,113,110,111,95,52,95,50,54,0),974]]);
         release_npP += "2";
         stationu += (inouttargetredx == String.fromCharCode(71,0) ? defaultplayerimgv.size : inouttargetredx.length);
         placeholderF = (inouttargetredx.length - stationu) > 21.22;
         defaultplayerimgv.set(inouttargetredx, defaultplayerimgv.size % 2);
      }
      while (foregroundH != String.fromCharCode(98,0)) {
          let saveG = String.fromCharCode(116,95,49,50,95,102,108,111,97,116,105,110,103,0);
          let combine9 = 3.0;
          let auto_eo8 = 0;
          let sliderE = String.fromCharCode(115,97,109,112,108,101,114,97,116,101,115,95,112,95,55,54,0);
          let inactive_ = String.fromCharCode(107,95,49,48,95,114,112,99,103,101,110,0);
         release_npP += "1";
         saveG = "2";
         combine9 /= Math.max(4, parseFloat(`${1}`));
         auto_eo8 &= 2;
         sliderE = `${parseInt(`${combine9}`) ^ auto_eo8}`;
         inactive_ += `${1 - sliderE.length}`;
         break;
      }
         foregroundH += `${foregroundH.length}`;
         release_npP = `${release_npP.length}`;
       let sidet = 1.0;
       let libflipperD = 4.0;
         libflipperD *= parseInt(`${sidet}`);
      event8 -= parseFloat(`${reactnativejsq.length}`);
       let renewN: Array<any> = [974, 788, 24];
          let controlsw = String.fromCharCode(100,101,102,101,114,95,53,95,56,51,0);
          let iconedity = String.fromCharCode(118,97,97,99,97,108,99,117,108,97,116,105,111,110,95,101,95,53,48,0);
         renewN.push((iconedity == String.fromCharCode(121,0) ? controlsw.length : iconedity.length));
          let dnewinterstitialc = 4.0;
         renewN = [1];
         dnewinterstitialc += parseFloat(`${parseInt(`${dnewinterstitialc}`) & 2}`);
      for (let r = 0; r < 3; r++) {
         renewN.push(renewN.length % (Math.max(2, renewN.length)));
      }
      gpayx *= parseFloat(`${humidityr.length - shrinkv.size}`);
