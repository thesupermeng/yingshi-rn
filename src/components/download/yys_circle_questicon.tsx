...
import VodImageCard from '../vod/yys_mail';
import VodDescription from '../vod/yys_statistics';
import { IConfirmationLibffmpegkit, yysStoreService } from '@type';
import FastImage from '../common/yys_alert_backwhite'
import { DownloadPauseYellowSvg } from '@static';
...
function DownloadVodCard({
  ...
  const isVodsDownloading = download.episodes.some(ep => ep.status === IConfirmationLibffmpegkit.ICrown)
  const isVodsDownloadingPaused = !isVodsDownloading && download.episodes.some(ep => ep.status === IConfirmationLibffmpegkit.IRightTerms || ep.status === IConfirmationLibffmpegkit.IRedirectLibyoga)
