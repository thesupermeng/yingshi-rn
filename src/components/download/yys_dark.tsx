...
import VodImageCard from '../vod/yys_render_orientation';
import VodDescription from '../vod/yys_unselected';
import { MARTextinputBackground, yys_LibsgcorePath } from '@type/yys_fast';
import FastImage from '../common/yys_vertical_collection'
import DownloadPauseYellowIcon from '@static/images/activityCombine.svg'
...
function DownloadVodCard({
  ...
  const isVodsDownloading = download.episodes.some(ep => ep.status === MARTextinputBackground.MARRewardvideoBell) 
  const isVodsDownloadingPaused = !isVodsDownloading && download.episodes.some(ep => ep.status === MARTextinputBackground.MARGpayMintegral || ep.status === MARTextinputBackground.MARInjuryModels)
