...
import VodImageCard from '../vod/wawa_bggradient';
import VodDescription from '../vod/wawa_iconarrowright_libcxxcomponents';
import { KVCarouselSort, wawaWhiteanimationlive } from '@type/wawa_emojiheart_alert';
import FastImage from '../common/wawa_iconarrowrightblack'
import DownloadPauseYellowIcon from '@static/images/rulesMembershipSport.svg'
...
interface wawaAwayShow {
  download: wawaWhiteanimationlive;
  onPress?: any;
  params?: any[];
...
function DownloadVodCard({
  ...
  const isVodsDownloading = download.episodes.some(ep => ep.status === KVCarouselSort.KVIconchatsend) 
  const isVodsDownloadingPaused = !isVodsDownloading && download.episodes.some(ep => ep.status === KVCarouselSort.KVFlipperForward || ep.status === KVCarouselSort.KVSharemodal)
