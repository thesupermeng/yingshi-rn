...
import VodImageCard from '../vod/tt_country_math';
import VodDescription from '../vod/tt_indicator';
import { ZRChinasame, ttTumbnail } from '@type/tt_const_videocommon';
import FastImage from '../common/tt_connection'
import DownloadPauseYellowIcon from '@static/images/gemfileClubDropdown.svg'
...
function DownloadVodCard({
  ...
  const isVodsDownloading = download.episodes.some(ep => ep.status === ZRChinasame.ZRSelectedBaidu) 
  const isVodsDownloadingPaused = !isVodsDownloading && download.episodes.some(ep => ep.status === ZRChinasame.ZRGoal || ep.status === ZRChinasame.ZRBackwardTemp)
