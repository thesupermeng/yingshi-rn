...
import VodImageCard from '../vod/mayi_control';
import VodDescription from '../vod/mayi_libavutil_progress';
import { CUASheetIconarrowright, mayi_CurrentFootball } from '@type/mayi_agreement';
import FastImage from '../common/mayi_slider'
import DownloadPauseYellowIcon from '@static/images/roomGreytickFootballfield.svg'
...
function DownloadVodCard({
  ...
  const isVodsDownloading = download.episodes.some(ep => ep.status === CUASheetIconarrowright.CUAAudienceUmeng) 
  const isVodsDownloadingPaused = !isVodsDownloading && download.episodes.some(ep => ep.status === CUASheetIconarrowright.CUAFlipperTurndown || ep.status === CUASheetIconarrowright.CUATextlayoutmanagerLibreactperfloggerjni)
