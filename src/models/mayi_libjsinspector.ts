export class mayi_Gift {
    ...
    public static fromJson = (json: any): mayi_Gift => {
        return new mayi_Gift({
    ...
    public static isGuest = (user?: mayi_Gift | null): boolean => {
        if (user === null || user === undefined) return true;

        return user.userEmail === '' && user.userPhoneNumber === '';
    }

    public static isLogin = (user?: mayi_Gift | null): boolean => {
        if (user === null || user === undefined) return false;

        return user.userEmail !== '' || user.userPhoneNumber !== '';
    }

    public static isVip = (user?: mayi_Gift | null): boolean => {
        if (user === null || user === undefined) return false;
