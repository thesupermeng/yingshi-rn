export class wwBodan {
    ...
    public static fromJson = (json: any): wwBodan => {
        return new wwBodan({
    ...
    public static isGuest = (user?: wwBodan | null): boolean => {
        if (user === null || user === undefined) return true;

        return user.userEmail === '' && user.userPhoneNumber === '';
    }

    public static isLogin = (user?: wwBodan | null): boolean => {
        if (user === null || user === undefined) return false;

        return user.userEmail !== '' || user.userPhoneNumber !== '';
    }

    public static isVip = (user?: wwBodan | null): boolean => {
        if (user === null || user === undefined) return false;

        return user.userCurrentTimestamp < user.userMemberExpired;
    }

    public static fakeIsLogin = (user?: wwBodan | null): boolean => {
        return true;
    }

    public static fakeIsVip = (user?: wwBodan | null): boolean => {
        return true;
    }
}