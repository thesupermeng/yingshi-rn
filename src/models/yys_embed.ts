export class yysIconstar {
    ...
    public static fromJson = (json: any): yysIconstar => {
        return new yysIconstar({
    ...
    public static isGuest = (user?: yysIconstar | null): boolean => {
        if (user === null || user === undefined) return true;

        return user.userEmail === '' && user.userPhoneNumber === '';
    }

    public static isLogin = (user?: yysIconstar | null): boolean => {
        if (user === null || user === undefined) return false;

        return user.userEmail !== '' || user.userPhoneNumber !== '';
    }

    public static isVip = (user?: yysIconstar | null): boolean => {
        if (user === null || user === undefined) return false;
