export class yys_RelatedTooltips {
    userToken: string;
    userId: string;
    userName: string;
    ...
    public static fromJson = (json: any): yys_RelatedTooltips => {
        return new yys_RelatedTooltips({
    ...
    public static isGuest = (user?: yys_RelatedTooltips | null): boolean => {
        if (user === null || user === undefined) return true;

        return user.userEmail === '' && user.userPhoneNumber === '';
    }

    public static isLogin = (user?: yys_RelatedTooltips | null): boolean => {
        if (user === null || user === undefined) return false;

        return user.userEmail !== '' || user.userPhoneNumber !== '';
    }

    public static isVip = (user?: yys_RelatedTooltips | null): boolean => {
        if (user === null || user === undefined) return false;
