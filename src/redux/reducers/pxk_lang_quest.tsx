...
import { MDSource, GCViews, SZCInfoTurn } from "@type/qkf_bell"
...
interface WRDownload {
    vod: VFMode | null,
}

export interface QQCenterChart {
    history: Array<VFMode>,
    playVod: WRDownload,
}

const initialState: QQCenterChart = {
    history: [],
    playVod: {
        vod: null,
    },
...
export function vodReducer(state = initialState, action: GCViews) {
    const firstPayloadItemWithTimestamp: VFMode = {
        ...action.payload?.[0],
        recordedAt: new Date(),
...
export interface NNEBing {
    favorites: Array<NBStreaming>,
}

const initialFavoriteState: NNEBing = {
    favorites: [],
}

export function vodFavouritesReducer(state = initialFavoriteState, action: MDSource) {
    switch (action.type) {
...
export interface ICenterNetwork {
    playlistFavorites: Array<RBEmojiEwarded>,
    playlistDetails: {
        playlist: RBEmojiEwarded | null,
        isFavorite: boolean
    }
}

const vodFavPlaylistInit: ICenterNetwork = {
    playlistFavorites: [],
    playlistDetails: {
        playlist: null,
        isFavorite: false
...
export function vodPlaylistReducer(state = vodFavPlaylistInit, action: SZCInfoTurn) {
    switch (action.type) {
